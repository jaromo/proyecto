...
if(params$cliente%in%c("BANCO POPULAR","BCR")){
  cat="BANCA"
  cat2=c("CAPTACION DINERO","SEGUROS","TARJETAS CREDITO","BANCA ELECTR CNCPT",
...
if(params$cliente%in%c("BCR","BANCO POPULAR","Rostipollos","Pizza Hut")){
...
if(params$cliente%in%c("BANCO POPULAR","INS","BCR")){
...
if(params$moneda=="Dólares"){
  for(i in 1:length(levels(data$Anunciante))){
    ...
    x3[i]=sum(data$INV.Dólar[(data$Anunciante==levels(data$Anunciante)[i]) & data$Año==as.numeric(params$ano)-1 & data$Mes<=params$mes_f & data$Mes>=params$mes_i]) 
  }
}else{
  for(i in 1:length(levels(data$Anunciante))){
    x2[i]=sum(data$INV[(data$Anunciante==levels(data$Anunciante)[i]) & data$Año==params$ano & data$Mes<=params$mes_f & data$Mes>=params$mes_i]) 
     x3[i]=sum(data$INV[(data$Anunciante==levels(data$Anunciante)[i]) & data$Año==as.numeric(params$ano)-1 & data$Mes<=params$mes_f & data$Mes>=params$mes_i]) 
  }
}
...
names(x2)=c("Marca","Anterior","Actual","Variacion")
...
<div align="center" style="float:right;width:35%;height:50%">`r img(src=paste('https://camedia1.shinyapps.io/Reportes/',as.character(params$cliente),'.png',sep=""))`</div><div align="center" style="float:left;width:35%;height:3"><br><br>`r img(src='https://camedia1.shinyapps.io/Reportes/camedia.png')`
</div> 
<div style="clear:both"></div>
<br>
<br>
...
p=data.table(p)
if(params$moneda=="Colones"){
  p1=p[,list(Inversión=sum(INV)),by=Categoría]
}else{
  p=p[,list(Inversión=sum(INV.Dólar)),by=Categoría]
}
p=p[order(-p$Inversión),]
...
if(params$moneda=="Colones"){
  for(i in 1:params$mes_f){
    if(i%in%base1$Mes){
    his1[i]=sum(base1$INV[base1$Categoría%in%cat2 & base1$Año==as.numeric(params$ano)-1 & base1$Mes==i])
    }else{
      his1[i]=0
    }
  }
  ...
  for(i in 1:params$mes_f){
    if(i%in%base1$Mes){
    his1[i]=sum(base1$INV.Dólar[base1$Categoría%in%cat2 & base1$Año==as.numeric(params$ano)-1 & base1$Mes==i])
    }else{
      his1[i]=0
    }
  }    
...
names(his)=c("Mes",paste("Inversión",as.numeric(params$ano)-1),paste("Inversión",params$ano),"Variacion")
...
<div style="clear:both"></div>
<br>
En este periodo la principal categoría es **`r p$Categoría[1]`**, con una inversión de `r p$Inversión[1]` `r moneda`
...
cat(as.character(p$Categoría[1]))
```
</h3></center>
...
data1=data[data$Categoría==as.character(p$Categoría[1]) & data$Año==params$ano & data$Mes<=params$mes_f & data$Mes>=params$mes_i,]
...
data1=data1[order(data1$Inversión),]
data1$Anunciante = factor(data1$Anunciante, levels=unique(data1$Anunciante))
temp=""
if(length(data1$Anunciante)>3){
  for(i in 1:3){
    temp2=paste(data1$Anunciante[i],paste(format(x2$Actual[i],big.mark = ",")," ",moneda),sep=": ")
    temp2=paste(temp2," ","(",x2$Variacion[i],")",sep="")
    if(3==i){
      temp3=" y "
    }else{
      if(i==1){
...
cat("La inversión registrada en ",cat," de ",params$mes_in," a ",params$mes_fin," ",params$`ano`,", se divide en ",temp)
```
...
if("TV"%in%data_2$Tipo.de.Medio){
  data_3=data_1[data_1$Tipo.de.Medio=="TV" & data_1$Año== as.numeric(params$ano),]
  data_5=data_3
...
cat(as.character(p$Categoría[2]))
```
</h3></center>
...
```{r echo=FALSE, eval= flag2[1]}
data1=data[data$Categoría==as.character(p$Categoría[2]) & data$Año==params$ano & data$Mes<=params$mes_f & data$Mes>=params$mes_i,]
...
cat(as.character(p$Categoría[3]))
```
</h3></center>
...
```{r echo=FALSE, eval= flag2[1]}
data1=data[data$Categoría==as.character(p$Categoría[3]) & data$Año==params$ano & data$Mes<=params$mes_f & data$Mes>=params$mes_i,]
data1=data.table(data1[data1$Anunciante%in%x2$Marca[1:3],])
data1=data1[,list(Inversión=sum(INV.Dólar)),by=list(Anunciante,Tipo.de.Medio)]
data1[ , `Porcentaje` := round(`Inversión` / sum( `Inversión` ) * 100,2) , by = list(Anunciante) ]
data1$Anunciante = factor(data1$Anunciante, levels=unique(x2$Marca[3:1]))
