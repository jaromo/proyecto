...
p=data.table(p)
if(params$moneda=="Colones"){
  p=p[,list(Inversión=INV),by=Categoría]
}else{
  p=p[,list(Inversión=INV.Dólar),by=Categoría]
}
p=p[order(-p$Inversión),]
p$Inversión=format(p$Inversión,big.mark = ",")
kable(head(p,10),format="html",row.names = F)%>%
...
```
<br>
La inversión registrada en **`r cat`** de `r params$mes_in` a `r params$mes_fin` `r params$ano`, se divide en `r temp`
<br>
