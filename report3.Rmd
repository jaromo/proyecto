...
names(base2)=c("Año","Mes","Categoría","INV.Dólar","INV")
data=params$base
base_ins=params$base3
base_ins1=data.table(base_ins[base_ins$Ano==as.numeric(params$ano) & base_ins$MES<=params$mes_f & base_ins$MES>=params$mes_i,])
base_ins2=base_ins1[,list(INV=sum(INV.Dolar)),by=list(AGENCIA,tipo_medio,dmedio)]
base_ins2$AGENCIA=as.factor(as.character(base_ins2$AGENCIA))
flag4=c(F,F,F)
if("OGILVY"%in%levels(base_ins2$AGENCIA)){
  flag4[1]=T
}
if("LA TRES"%in%levels(base_ins2$AGENCIA)){
  flag4[2]=T
}
if("JBQ"%in%levels(base_ins2$AGENCIA)){
  flag4[3]=T
}
...
data$Anunciante[data$Anunciante%in%c("BLUE CROSS BLUE SHIELD","TRIPLE S BLUE INC")]="TRIPLE S BLUE INC"
data$Anunciante[data$Anunciante%in%c("ASSA SEGUROS","ASEGURADORA ASSA INT")]="ASSA SEGUROS"
...
base2$INV=base1$INV/1000
base2$INV.Dólar=base1$INV.Dólar/1000
}
...
if(if(length(which(x2$Marca==marcacliente))==0){0}else{which(x2$Marca==marcacliente)}>3){
  x3=x3[c(1:3,which(x3$Marca==marcacliente)),]
}else{
  x3=x3[1:3,]
}
...
if(params$moneda=="Colones"){
  for(i in 1:params$mes_f){
    if(i%in%base2$Mes){
    his[i]=sum(base2$INV[base2$Categoría%in%cat2 & base2$Año==params$ano & base2$Mes==i])
    }else{
      his[i]=0
    }
  }
  ...
  for(i in 1:params$mes_f){
    if(i%in%base2$Mes){
    his[i]=sum(base2$INV.Dólar[base2$Categoría%in%cat2 & base2$Año==params$ano & base2$Mes==i])
    }else{
      his[i]=0
    }
  }    
...
if(params$moneda=="Colones"){
  for(i in 1:params$mes_f){
    if(i%in%base2$Mes){
    his1[i]=sum(base2$INV[base2$Categoría%in%cat2 & base2$Año==as.numeric(params$ano)-1 & base2$Mes==i])
    }else{
      his1[i]=0
    }
  }
  ...
  for(i in 1:params$mes_f){
    if(i%in%base2$Mes){
    his1[i]=sum(base2$INV.Dólar[base2$Categoría%in%cat2 & base2$Año==as.numeric(params$ano)-1 & base2$Mes==i])
    }else{
      his1[i]=0
    }
  }    
...
if(length(x2$Marca)>3){
  if(if(length(which(x2$Marca==marcacliente))==0){0}else{which(x2$Marca==marcacliente)}>3){
    ...
    for(i in c(1:3,which(x2$Marca==as.character(marcacliente)))){
      temp2=paste(x2$Marca[i],paste(x2$Porcentaje[i],"%"),sep=": ")
      if(3==i){
        temp3=" y "
      }else{
...
if(nrow(x)>=1){
  kable(fra[[1]],format = "html",row.names = F)%>%
    kable_styling(bootstrap_option="striped",full_width = F,font_size = 12)
}
```
</td>
<td align="center" style="width:25%">
...
if("PUBLICIDAD EXTERIOR"%in%data_2$Tipo.de.Medio){
  ...
  for(i in 1:cont){
    table[[i]]=kable(data_4[data_4$Anunciante%in%levels(data_4$Anunciante)[i],-c(1,2,4)],format = "html",row.names = F)%>%
      kable_styling(bootstrap_option="striped",full_width = T,font_size = 12)
    anun[i]=as.character(levels(data_4$Anunciante)[i])}
}else{
...
<center><h3>**Análisis por AGENCIA**</h3></center>
<br>
<br>
<center>
```{r echo=FALSE,fig.height=4,eval=flag4[1]}
img(src='https://camedia.shinyapps.io/Reportes/OGILVY.png')
```
</center>
<br>
<br>
...
<tr>
<td>
```{r echo=FALSE,eval=flag4[1]}
ogilvy=base_ins2[base_ins2$AGENCIA=="OGILVY",-1]
ogilvy_tm=ogilvy[,list(INV=sum(INV)),by=tipo_medio]
ogilvy_tm=ogilvy_tm[order(-ogilvy_tm$INV),]
ogilvy$tipo_medio=factor(ogilvy$tipo_medio,levels = ogilvy_tm$tipo_medio)
ogilvy=ogilvy[order(ogilvy$tipo_medio,-ogilvy$INV),]
names(ogilvy)=c("TIPO DE MEDIO","DESCRIPCION DE MEDIO","INVERSION")
kable(ogilvy,"html")%>%
  collapse_rows(1)%>%
...
<td>
```{r echo=FALSE,eval=flag4[1]}
ogilvy_tm$INV=round(ogilvy_tm$INV/sum(ogilvy_tm$INV)*100,0)
ogilvy_tm=ogilvy_tm[ogilvy_tm$INV>0,]
ogilvy_tm$tipo_medio=as.factor(as.character(ogilvy_tm$tipo_medio))
ogilvy_tm$tipo_medio=factor(ogilvy_tm$tipo_medio,levels = ogilvy_tm$tipo_medio)
ggplot(data=ogilvy_tm,aes(x=ogilvy_tm$tipo_medio,y=ogilvy_tm$INV, fill=ogilvy_tm$tipo_medio))+
  geom_bar(position = "stack",stat = "identity")+
  geom_text(aes(label=paste(INV,"%")),position =position_stack(vjust=0.5) )+
  scale_fill_manual(values=brewer.pal(n = 8, name = "Set2"))+
  theme(legend.title = element_text(colour = "white"),
        legend.position="bottom",axis.text.y= element_text(colour = "white"),
        panel.background = element_rect(fill='white'),
        legend.text = element_text(size=10),axis.text.x = element_text(colour="white"),
        legend.key.size = unit(0.5,"cm"), axis.ticks.y = element_line(color = "white"),
...
<br>
<center>
```{r echo=FALSE,fig.height=4,eval=flag4[2]}
img(src='https://camedia.shinyapps.io/Reportes/LA TRES.png')
```
</center>
<br>
<br>
...
<tr>
<td>
```{r echo=FALSE,eval=flag4[2]}
la_tres=base_ins2[base_ins2$AGENCIA=="LA TRES",-1]
la_tres_tm=la_tres[,list(INV=sum(INV)),by=tipo_medio]
la_tres_tm=la_tres_tm[order(-la_tres_tm$INV),]
la_tres$tipo_medio=factor(la_tres$tipo_medio,levels = la_tres_tm$tipo_medio)
la_tres=la_tres[order(la_tres$tipo_medio,-la_tres$INV),]
names(la_tres)=c("TIPO DE MEDIO","DESCRIPCION DE MEDIO","INVERSION")
kable(la_tres,"html")%>%
  collapse_rows(1)%>%
  kable_styling(bootstrap_options = "bordered",full_width = F,font_size = 12)
```
...
<td>
```{r echo=FALSE,eval=flag4[2]}
la_tres_tm$INV=round(la_tres_tm$INV/sum(la_tres_tm$INV)*100,0)
la_tres_tm=la_tres_tm[la_tres_tm$INV>0,]
la_tres_tm$tipo_medio=as.factor(as.character(la_tres_tm$tipo_medio))
la_tres_tm$tipo_medio=factor(la_tres_tm$tipo_medio,levels = la_tres_tm$tipo_medio)
ggplot(data=la_tres_tm,aes(x=la_tres_tm$tipo_medio,y=la_tres_tm$INV, fill=la_tres_tm$tipo_medio))+
  geom_bar(position = "stack",stat = "identity")+
  geom_text(aes(label=paste(INV,"%")),position =position_stack(vjust=0.5) )+
  scale_fill_manual(values=brewer.pal(n = 8, name = "Set2"))+
  theme(legend.title = element_text(colour = "white"),
...
<br>
<center>
```{r echo=FALSE,fig.height=4,eval=flag4[3]}
img(src='https://camedia.shinyapps.io/Reportes/JBQ.png')
```
</center>
<br>
<br>
...
<tr>
<td>
```{r echo=FALSE,eval=flag4[3]}
jbq=base_ins2[base_ins2$AGENCIA=="JBQ",-1]
jbq_tm=jbq[,list(INV=sum(INV)),by=tipo_medio]
jbq_tm=jbq_tm[order(-jbq_tm$INV),]
jbq$tipo_medio=factor(jbq$tipo_medio,levels = jbq_tm$tipo_medio)
jbq=jbq[order(jbq$tipo_medio,-jbq$INV),]
names(jbq)=c("TIPO DE MEDIO","DESCRIPCION DE MEDIO","INVERSION")
kable(jbq,"html")%>%
  collapse_rows(1)%>%
  kable_styling(bootstrap_options = "bordered",full_width = F,font_size = 12)
```
...
<td>
```{r echo=FALSE,eval=flag4[3]}
jbq_tm$INV=round(jbq_tm$INV/sum(jbq_tm$INV)*100,0)
jbq_tm=jbq_tm[jbq_tm$INV>0,]
jbq_tm$tipo_medio=as.factor(as.character(jbq_tm$tipo_medio))
jbq_tm$tipo_medio=factor(jbq_tm$tipo_medio,levels = jbq_tm$tipo_medio)
ggplot(data=jbq_tm,aes(x=jbq_tm$tipo_medio,y=jbq_tm$INV, fill=jbq_tm$tipo_medio))+
  geom_bar(position = "stack",stat = "identity")+
  geom_text(aes(label=paste(INV,"%")),position =position_stack(vjust=0.5) )+
  scale_fill_manual(values=brewer.pal(n = 8, name = "Set2"))+
  theme(legend.title = element_text(colour = "white"),
...
img(src='https://camedia.shinyapps.io/Reportes/AGENCIAS.png')
```
</center>
<br>
<br>
<center>
```{r echo=F, message=FALSE,fig.width=2}
base_ins3=base_ins1[,list(INV=sum(INV.Dolar)),by=list(tipo_medio,MES)]
base_ins3$MES=recode_factor(base_ins3$MES,`1`="Enero",`2`="Febrero",`3`="Marzo",`4`="Abril",`5`="Mayo",`6`="Junio",`7`="Julio",`8`="Agosto",`9`="Septiembre",`10`="Octubre",`11`="Noviembre",`12`="Diciembre")
base_ins4=dcast(base_ins3,tipo_medio~MES)
base_ins4$TOTAL=apply(base_ins4[,-1],1,sum,na.rm=T)
base_ins4$SHARE=round(base_ins4$TOTAL/sum(base_ins4$TOTAL)*100,2)
base_ins4=base_ins4[order(-base_ins4$TOTAL),]
func <- function(z) if (is.numeric(z)) sum(z,na.rm=T) else NA
tem=data.frame(lapply(base_ins4,func))
tem[is.na(tem)]="TOTAL GENERAL"
base_ins4=rbind(base_ins4,tem)
base_ins4$SHARE=paste(base_ins4$SHARE,"%",sep="")
base_ins4=data.frame(base_ins4)
base_ins4[is.na(base_ins4)]=0
for(i in 2:(ncol(base_ins4)-1)){
  base_ins4[,i]=format(base_ins4[,i],big.mark = ",",na.rm=T)
}
func<-function(z) {
  x=logical(length(z))
  x[grep("  0",z)]=T
...
base_ins4[as.matrix(data.frame(lapply(base_ins4,func)))]="-"
names(base_ins4)[1]="TIPO DE MEDIO"
kable(base_ins4,"html")%>%
  collapse_rows(1)%>%
  kable_styling(full_width = F,font_size = 12)%>%
  row_spec(c(0,nrow(base_ins4)), bold = T, color = "white", background = "#026324")
```
</center>
<br>
<center>
```{r echo=F}
base_ins5=base_ins3[,`Porcentaje` := `INV` / sum( `INV` ) * 100,by=MES]
ggplot(data=base_ins5,aes(x=MES,y=Porcentaje, fill=tipo_medio))+
  geom_bar(position = "stack",stat = "identity")+
  geom_text(aes(label=paste(round(Porcentaje,0),"%")),position =position_stack(vjust=0.5) )+
  scale_fill_manual(values=brewer.pal(n = 8, name = "Set2"))+
  theme(legend.title = element_text(colour = "white"),
        legend.position="bottom",axis.text.y= element_text(colour = "white"),
        panel.background = element_rect(fill='white'),
